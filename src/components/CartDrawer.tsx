    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display italic">Panier</SheetTitle>
          <SheetDescription>
            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-display text-sm italic truncate">{item.product.node.title}</h4>
                        <p className="text-xs text-muted-foreground font-body">{item.selectedOptions.map(o => o.value).join(' • ')}</p>
                        <p className="font-body font-semibold text-sm">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(0)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-body text-xl font-bold">{items[0]?.price.currencyCode} {totalPrice.toFixed(0)}</span>
                </div>
                <Button onClick={handleCheckout} variant="hero" className="w-full" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
